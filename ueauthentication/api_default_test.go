...
func TestUeAuthenticationsPost(t *testing.T) {
	...
	case models.AuthType_EAP_AKA_PRIME:
		...
		if testSuccess == 1 {
			...
			atRes, _ = producer.EapEncodeAttribute("AT_RES", TestUEAuth.TestUeEapAuthTable[TestUEAuth.FAILURE_CASE].Res)
		}
		// calculate AT_MAC
		fmt.Println("---------------------")
		...
		MACvalue := producer.CalculateAtMAC([]byte(K_autDe), encodedPktBeforeMAC)
		...
		if rtnContent.Code == layers.EAPCodeSuccess {
			...
			eapPkt2 := producer.ConstructEapNoTypePkt(radius.EapCodeResponse, rtnContent.Id)
